...
| Local File Index | Persistent Everything-like metadata index for bulk search and whole-drive analysis. Large scans prefer the index, ask before starting maintenance, and can summarize recursive usage by top-level directory without rereading the disk every time |
...
- Fast elevated NTFS/MFT indexing is isolated in a short-lived read-only helper and requires explicit approval before UAC is shown
